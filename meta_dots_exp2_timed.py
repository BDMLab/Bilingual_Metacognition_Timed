...
Instructions1Text =visual.TextStim(win, text='You will see two circles on the screen, each with a number of dots inside. Your task is to try to guess which circle contains the most dots. Then we will ask you to rate your confidence in your decision. \n\n\nPlease press space to see some example stimuli.',
    units='cm', pos=(0,2.0), height=1, wrapWidth=30)
...
FixationCross = visual.TextStim(win=win, ori=0, name='FixationCrossLeft',
    text=u'+', units='cm', pos=(0, 0), height=3, wrapWidth=10)
...
DotCalibrationInstructions = visual.TextStim(win,text="Which circle contains more dots?",
    units='cm', pos=(0,-8.0), height=1, wrapWidth=20)
    
#Instalize components for Routine Instructions2
Instructions2Clock =core.Clock()

Instructions2Text =visual.TextStim(win,text=u"The first part of the task is to choose which circle contains the most dots. Try to respond as quickly and as accurately as you can. Press the left arrow key \u2190 to pick the left circle and the right arrow key \u2192 to pick the right circle. After each trial you will get feedback and there will be a yellow box surrounding the option you pick. Don't worry if some of your decisions feel like guesses - it is a hard task! \n\n\nPress the space bar to continue.",
    units='cm', pos=(0,2.0),  height=1, wrapWidth=30)

# Instalize components for Feedback trials

FeedbackCorrect = visual.TextStim(win,text="Correct!",
    units='cm', pos=(0,-8.0), height=1, color='green', wrapWidth=15)

FeedbackIncorrect = visual.TextStim(win,text="Incorrect",
    units='cm', pos=(0,-8.0), height=1, color='red', wrapWidth=15)

FeedbackBoxLeft = visual.Rect(win=win, name='box',
    width=13, height=13,
    ori=0, pos=[-11, 4], units='cm',
    lineWidth=5, lineColor=u'yellow', lineColorSpace=u'rgb',
    fillColor=None, fillColorSpace=u'rgb',
    opacity=1,interpolate=True)
    
FeedbackBoxRight = visual.Rect(win=win, name='box',
    width=13, height=13,
    ori=0, pos=[11, 4], units='cm',
    lineWidth=5, lineColor=u'yellow', lineColorSpace=u'rgb',
    fillColor=None, fillColorSpace=u'rgb',
...
# Instalize components for Slow trials
TooSlow = visual.TextStim(win,text="Too Slow!",
    units='cm', pos=(0,0), height=3, color='#FFFFFF', wrapWidth=15)
...
# Instalize components for ConfFeedback trials
ConfFeedbackClock =core.Clock()
...
RestText = visual.TextStim(win=win, ori=0, name='rest_prompt_txt', text=u'Great! Now take a rest and press spacebar when you are ready to begin the next block.', font=u'Arial', pos=[0, 0], height=0.08, wrapWidth=None, color=u'white', colorSpace=u'rgb', opacity=1, depth=0.0)

# Instalize components for "Thank You"
ThankYouClock = core.Clock()
ThankYouText = visual.TextStim(win=win, ori=0, name='rest_prompt_txt', text=u'You have now completed this task. Thank you for your participation. Please inform the experimenter that you have finished.', font=u'Arial', pos=[0, 0], height=0.08, wrapWidth=None, color=u'white', colorSpace=u'rgb', opacity=1, depth=0.0)
...
### Timings
ITI = 2
DotPresentationTime = 1.5
PreResponseTime = 0
ConfidenceDisplayTime = 1.0
ConfidenceResponseTime = 1.0
ConfidenceFeedbackTime = 1.0
FeedbackITI = 0
FeedbackTime = 1
SlowITI = 0
SlowTime = 1
...
while continueRoutine:
    ...
    if t >= Introduction_Wait and IntroductionResponse.status == NOT_STARTED:
        # keep track of start time/frame for later
...
while continueRoutine:
    ...
    if t >= Introduction_Wait and Instructions1Response.status == NOT_STARTED:
        # keep track of start time/frame for later
...
for thischoice_loop in choice_loop:
    ...
    ExampleClock.reset()
    frameN = -1
    routineTimer.add(ITI + DotPresentationTime)
     # update component parameters for each repeat
    DotPatchLeft=visual.DotStim(win, color=(1.0,1.0,1.0), dir=270, units='cm',
    nDots=int(ExDotLeft), fieldShape='circle', fieldPos=(-11.0, 4.0), fieldSize=11,
    dotSize= 15,
    dotLife=-1, #number of frames for each dot to be drawn
    signalDots='same', #are the signal dots the 'same' on each frame? (see Scase et al)
    noiseDots='position', #do the noise dots follow random- 'walk', 'direction', or 'position'
    ...
    nDots=int(ExDotRight), fieldShape='circle', fieldPos=(11, 4.0),fieldSize=11,
    dotSize= 15,
    dotLife=-1, #number of frames for each dot to be drawn
    signalDots='same', #are the signal dots the 'same' on each frame? (see Scase et al)
    noiseDots='position', #do the noise dots follow random- 'walk', 'direction', or 'position'
    ...
    dot_choiceComponents.append(FixationCross)
 ...
 #-------Start Routine "dot_choice"-------
    ...
    while continueRoutine:
        ...
        # *CircleLeft* updates
        if t >= Beginning_ITI and CircleLeft.status == NOT_STARTED:
            CircleLeft.tStart = t
            CircleLeft.frameNStart = frameN
            CircleLeft.setAutoDraw(True)
        if CircleLeft.status == STARTED and t >= DotPresentationTime + Beginning_ITI:
        ...
        if FixationCross.status == STARTED and t >= Beginning_ITI:
            FixationCross.setAutoDraw(False)
            
        # *CircleRight* updates
        if t >= Beginning_ITI and CircleRight.status == NOT_STARTED:
            CircleRight.tStart = t
            CircleRight.frameNStart = frameN
            CircleRight.setAutoDraw(True)
        if CircleRight.status == STARTED and t >= DotPresentationTime + Beginning_ITI:
            CircleRight.setAutoDraw(False)


            
         # *DotPatchLeft* updates
        if t >= Beginning_ITI and DotPatchLeft.status == NOT_STARTED:
            DotPatchLeft.tStart = t
            DotPatchLeft.frameNStart = frameN
            DotPatchLeft.setAutoDraw(True)
        if DotPatchLeft.status == STARTED and t >= DotPresentationTime + Beginning_ITI:
            DotPatchLeft.setAutoDraw(False)

        
        # *DotPatchRight* updates
        if t >= Beginning_ITI and DotPatchRight.status == NOT_STARTED:
            DotPatchRight.tStart = t
            DotPatchRight.frameNStart = frameN
            DotPatchRight.setAutoDraw(True)
        if DotPatchRight.status == STARTED and t >= DotPresentationTime + Beginning_ITI:
            DotPatchRight.setAutoDraw(False)

            
                # *DotExampleLeft* updates
        if t >= Beginning_ITI and DotExampleLeft.status == NOT_STARTED:
            DotExampleLeft.tStart = t
            DotExampleLeft.frameNStart = frameN
            DotExampleLeft.setAutoDraw(True)
        if DotExampleLeft.status == STARTED and t >= DotPresentationTime + Beginning_ITI:
            DotExampleLeft.setAutoDraw(False)

                # *DotExampleRight* updates
        if t >= Beginning_ITI and DotExampleRight.status == NOT_STARTED:
            DotExampleRight.tStart = t
            DotExampleRight.frameNStart = frameN
            DotExampleRight.setAutoDraw(True)
        if DotExampleRight.status == STARTED and t >= DotPresentationTime + Beginning_ITI:
            DotExampleRight.setAutoDraw(False)
...
while continueRoutine:
    ...
    if t >= Instruction_Wait and Instructions2Response.status == NOT_STARTED:
        # keep track of start time/frame for later
...
#-------Start Routine "dot_choice"-------
    ...
    while continueRoutine:
        ...
        # *CircleLeft* updates
        if t >= ITI and CircleLeft.status == NOT_STARTED:
            CircleLeft.tStart = t
            CircleLeft.frameNStart = frameN
            CircleLeft.setAutoDraw(True)
        ...
        if FixationCross.status == NOT_STARTED:
            ...
            FixationCross.frameNStart = frameN
            FixationCross.setAutoDraw(True)
        if FixationCross.status == STARTED and t >= ITI:
            FixationCross.setAutoDraw(False)

        # *CircleRight* updates
        if t >= ITI and CircleRight.status == NOT_STARTED:
            CircleRight.tStart = t
            CircleRight.frameNStart = frameN
            CircleRight.setAutoDraw(True)
         ...
         # *DotPatchLeft* updates
        if t >= ITI and DotPatchLeft.status == NOT_STARTED:
            DotPatchLeft.tStart = t
            DotPatchLeft.frameNStart = frameN
            DotPatchLeft.setAutoDraw(True)

        # *DotPatchRight* updates
        if t >= ITI and DotPatchRight.status == NOT_STARTED:
            DotPatchRight.tStart = t
            DotPatchRight.frameNStart = frameN
            DotPatchRight.setAutoDraw(True)

        # *DotExampleInstructions* updates
        if t >= ITI and DotCalibrationInstructions.status == NOT_STARTED:
        ...
        if t >= ITI and KeyRespCal.status == NOT_STARTED:
            KeyRespCal.tStart = t
        ...
        # If time has run out, make the trial invalid, end the current routine and start the slow routine
        if t > ITI + DotPresentationTime:
            ValidChoice = False
            for thisComponent in dot_choiceComponents:
                if hasattr(thisComponent, 'status'):
            ...
            # store data for binary (TrialHandler)
            thisExp.addData('ValidChoice', ValidChoice)
    ...
    # If this is a valid trial give feedback:
    if ValidChoice == True:
        ...
        FeedbackComponents.append(FeedbackBoxLeft)
        FeedbackComponents.append(FeedbackBoxRight)
    ...
    #-------Start Routine "Feedback"-------
        ...
        while continueRoutine:
            ...
            if CircleLeft.status == NOT_STARTED:
                ...
                CircleLeft.frameNStart = frameN
                CircleLeft.setAutoDraw(True)
            elif CircleLeft.status == STARTED and t >= (FeedbackITI + FeedbackTime): #most of one frame period left
                CircleLeft.setAutoDraw(False)
                continueRoutine = False

            # *CircleRight* updates
            if CircleRight.status == NOT_STARTED:
                CircleRight.tStart = t
                CircleRight.frameNStart = frameN
                CircleRight.setAutoDraw(True)
            elif CircleRight.status == STARTED and t >= (FeedbackITI + FeedbackTime): #most of one frame period left
                CircleRight.setAutoDraw(False)

             # *DotPatchLeft* updates
            if DotPatchLeft.status == NOT_STARTED:
                DotPatchLeft.tStart = t
                DotPatchLeft.frameNStart = frameN
                DotPatchLeft.setAutoDraw(True)
            elif DotPatchLeft.status == STARTED and t >= (FeedbackITI + FeedbackTime): #most of one frame period left
                DotPatchLeft.setAutoDraw(False)

            # *DotPatchRight* updates
            if DotPatchRight.status == NOT_STARTED:
                DotPatchRight.tStart = t
                DotPatchRight.frameNStart = frameN
                DotPatchRight.setAutoDraw(True)
            elif DotPatchRight.status == STARTED and t >= (FeedbackITI + FeedbackTime): #most of one frame period left
                DotPatchRight.setAutoDraw(False)

            # *FeedbackCorrect* updates
            if KeyRespCal.corr == 1:
                if t >= FeedbackITI and FeedbackCorrect.status == NOT_STARTED:
                    FeedbackCorrect.tStart = t
                    FeedbackCorrect.frameNStart = frameN
                    FeedbackCorrect.setAutoDraw(True)
                elif FeedbackCorrect.status == STARTED and t >= (FeedbackITI + FeedbackTime): #most of one frame period left
                    FeedbackCorrect.setAutoDraw(False)

            # *FeedbackIncorrect* updates
            if KeyRespCal.corr == 0:
                if t >= FeedbackITI and FeedbackIncorrect.status == NOT_STARTED:
                       FeedbackIncorrect.tStart = t
                       FeedbackIncorrect.frameNStart = frameN
                       FeedbackIncorrect.setAutoDraw(True)
                elif FeedbackIncorrect.status == STARTED and t >= (FeedbackITI + FeedbackTime): #most of one frame period left
                    FeedbackIncorrect.setAutoDraw(False)

            # *FeedbackBoxLeft* updates
            if KeyRespCal.keys == 'left':
                if FeedbackBoxLeft.status == NOT_STARTED:
                    FeedbackBoxLeft.tStart = t
                    FeedbackBoxLeft.frameNStart = frameN
                    FeedbackBoxLeft.setAutoDraw(True)
                elif FeedbackBoxLeft.status == STARTED and t >= (FeedbackITI + FeedbackTime): #most of one frame period left
                    FeedbackBoxLeft.setAutoDraw(False)

            # *FeedbackBoxRight* updates
            if KeyRespCal.keys == 'right':
                if FeedbackBoxRight.status == NOT_STARTED:
                    FeedbackBoxRight.tStart = t
                    FeedbackBoxRight.frameNStart = frameN
                    FeedbackBoxRight.setAutoDraw(True)
                elif FeedbackBoxRight.status == STARTED and t >= (FeedbackITI + FeedbackTime): #most of one frame period left
                    FeedbackBoxRight.setAutoDraw(False)
    ...
    if ValidChoice == False:
        ...
        while continueRoutine:
            ...
            # *TooSlow* updates
            if t >= SlowITI and TooSlow.status == NOT_STARTED:
                TooSlow.tStart = t
                TooSlow.frameNStart = frameN
                TooSlow.setAutoDraw(True)
            elif TooSlow.status == STARTED and t >= (SlowITI + SlowTime): #most of one frame period left
                TooSlow.setAutoDraw(False)
...
while continueRoutine:
    ...
    # *Instructions2Response* updates
    if t >= Instruction_Wait and Instructions3Response.status == NOT_STARTED:
        # keep track of start time/frame for later
...
while continueRoutine:
    ...
    if t >= Instruction_Wait and Instructions4Response.status == NOT_STARTED:
        # keep track of start time/frame for later
...
#-------Start Routine "dot_choice"-------
    ...
    while continueRoutine:
        ...
        if t > ITI + DotPresentationTime:
            ...
            RestCounter += 1
            thisExp.addData('RestCounter', RestCounter)

            # Saving the staircase trial counter and the current expected total trials
            thisExp.addData('ValidTrials', MainStaircase.thisTrialN)
            thisExp.nextEntry()
    ...
    # If there has been 25 valid trials in this block, and we are not on the final block, show the rest screen
    if RestCounter == 25 and Block != 8:
        ...
        while continueRoutine:
            ...
            if t >= Rest_Wait and RestResponse.status == NOT_STARTED:
        ...
        if RestResponse.keys != None:
            ...
            # Update the RestCounter and the Block Counter
            RestCounter = 0
            Block +=1
